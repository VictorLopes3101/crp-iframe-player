window.addEventListener("message", function (e) {
	...
	for (var i = 0; i < video_config_media['streams'].length; i++) {
		if (video_config_media['streams'][i].format == 'trailer_hls' && video_config_media['streams'][i].hardsub_lang == user_lang) {
			if (rows_number <= 4) {
				video_m3u8_array.push(video_config_media['streams'][i].url.replace("clipTo/120000/", "clipTo/" + video_config_media['metadata']['duration'] + "/").replace(video_config_media['streams'][i].split("/")[2], "dl.v.vrv.co"));
				rows_number++;
			}
		}
		...
		complete: function (response) {
			...
			function download_ButtonClickAction() {
				...
				if(is_ep_premium_only == false) {
					video_dash_playlist_url_old = player_current_playlist.replace("master.m3u8","manifest.mpd").replace("v.vrv.co","dl.v.vrv.co");
					video_dash_playlist_url = player_current_playlist.replace("v.vrv.co","a-vrv.akamaized.net");
